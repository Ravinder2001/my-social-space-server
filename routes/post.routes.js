...
const { Post_Validations } = require("../helpers/body_validations/posts/validator");
...
const { UserIdValidation } = require("../helpers/db_validations/users.validations");
const { PostIdValidation } = require("../helpers/db_validations/post.validations");

const router = express.Router();
const storage = multer.memoryStorage();
const upload = multer({ storage });

router.post("/add", authentication, upload.array(File_Extension), Post_Validations.PostBody, Post_Validations.PostFile, Add_Post);
...
router.post("/comment/add/:post_id", authentication, Post_Validations.CommentBody, PostIdValidation, Add_Comment);
router.post("/like/add/:post_id", authentication, PostIdValidation, Add_Like);
router.delete("/like/delete/:post_id", authentication, PostIdValidation, Remove_Like);
...
router.put("/edit/:post_id", authentication, Post_Validations.EditPostBody, Edit_Post);
router.get("/like/getCount/:post_id", authentication, Get_Likes_Count);
